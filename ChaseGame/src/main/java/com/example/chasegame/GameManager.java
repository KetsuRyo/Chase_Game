...
public class GameManager {
    ...
    private void initializeAndPlacePieces(Player player, boolean isPlayerOne) {
        ...
        int[][] positions = isPlayerOne ?
                new int[][]{{0, 0}, {1, 1}, {1, 5}, {2, 2}, {2, 4}, {2, 0}, {2, 6}, {0, 6}} :
                new int[][]{{8, 6}, {7, 5}, {7, 1}, {6, 4}, {6, 2}, {6, 6}, {6, 0}, {8, 0}};

        // 创建并放置棋子
        for (int i = 0; i < pieceNames.length; i++) {
            GamePiece piece = new GamePiece(pieceNames[i], strengthLevels[i], positions[i][0], positions[i][1], "path/to/image.png"); // 根据实际情况更改图片路径
